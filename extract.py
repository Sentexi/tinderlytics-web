...
def extract(identifier):
    ...
    if os.path.exists(f'Data/{identifier}/myData.zip'):
        # Extract the zip file to a folder with the same name plus a timestamp
        timestamp = datetime.datetime.now().strftime("%d-%m-%Y")
        #currently not using the timestamp
        extraction_folder = f'Data/{identifier}/myData'
        with zipfile.ZipFile(f'Data/{identifier}/myData.zip', 'r') as zip_ref:
        ...
        if os.path.exists(json_file_path):
            ...
            os.remove(f'Data/{identifier}/myData.zip')
        else:
