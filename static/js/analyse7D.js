...
const currentURL7D = window.location.href;
const parts7D = currentURL7D.split('/');
const lastPart7D = parts7D[parts7D.length - 1];
const identifier7D = parseInt(lastPart7D);

fetch('../tinder7D/' + identifier7D)
  ...
  .then(csvData => {
	...
	// Calculate and populate match rate values
      ...
      var ctx = document.getElementById('TinderChart7D').getContext('2d');
